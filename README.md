...
![Agent Skill](https://img.shields.io/badge/Agent%20Skill-Claude%20Code%20%2F%20Cursor%20%2F%20Codex-7C3AED)

> 让 **AI 看懂 Mockplus(摹客)设计稿**的 Agent Skill。装到 Claude Code / Cursor / Codex 等 AI 后,你只需要把 Mockplus URL 贴给 AI,AI 就能拿到结构化 YAML + 切图 + 整页截图,直接生成 Vue / React / Flutter / 小程序代码 —— 不再让 AI 看 PNG 截图猜规格。

## 这不是 CLI,这是 Agent Skill
...
skill 本体在仓库的 `skills/mockplus-context/` 子目录里(自包含)。Claude Code 只扫描 `~/.claude/skills/<name>/SKILL.md` 这一层,所以**不能**直接把整个仓库 clone 进 skills 目录 —— 那样 `SKILL.md` 会被多套一层目录,扫不到。正确做法是 clone 后把内层 skill 目录软链进去(`git pull` 即可同步更新):
...
> Cursor / Codex / 其他 skill-aware AI 平台:把 `skills/mockplus-context/` 目录按各自平台的 Agent Skill 安装规范放进去即可。
