...
metadata:
  ...
  components:                          # SymbolInstance 反推的组件库注册表
    美孚门店通/icon/首页/面: { id, name, libraryName }
...
nodes:
  - id: <UUID>
    ...
    fills: fill_000001
    text: "合并转采购"
    textStyle: 01文字色1/16px/semibold/居中对齐 Style   # ← 设计师命名保留语义
    children: [...]

globalVars:
  styles:
    fill_000003:                       # 切图填充 — AI 看到这个会调 download
      - type: IMAGE
        imageRef: 2b417ea8...          # ← hash 跟 ./assets/<hash>.png 一一对应
        scaleMode: FILL
    layout_000007:
      mode: none
      locationRelativeToParent: { x: 266, y: 737 }
      dimensions: { width: 80, height: 22 }
    01文字色1/16px/semibold/居中对齐 Style:           # token key 是设计师命名,不是 hash
