...
Cookie 有效期约 30 天,过期重新 `set`。详见 `docs/cookie.md`。
...
- `scripts/mockplus.py` — CLI 入口(argparse 主分派)
- `scripts/_*.py` — 各子命令实现(`_api` / `_transform` / `_assets` / `_cookie` / `_tree` / `_schema`)
- `docs/api-reference.md` — 命令完整签名
- `docs/architecture.md` — 模块划分与字段语义
- `docs/cookie.md` — cookie 获取/配置详解
